...
impl Ext4File {
    ...
    */
    ...
    pub fn create_symlink(&mut self, target: &str, path: &str) -> Result<usize, i32> {
    ...
    pub fn readlink(&mut self, path: &str, buf: &mut [u8]) -> Result<usize, i32> {
        let c_path = CString::new(path).expect("CString::new failed");
        let c_path = c_path.into_raw();
        let mut rcnt: usize = 0;

        let r = unsafe { ext4_readlink(c_path, buf.as_mut_ptr() as *mut c_char, buf.len(), &mut rcnt) };
        unsafe {
            drop(CString::from_raw(c_path));
        }
